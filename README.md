...
This project shows a production-grade voice AI application for medical front office workflows, featuring real-time speech-to-text (STT), intent recognition, and text-to-speech (TTS) with HIPAA-minded design principles. The NLP models used are applied with Piper and Whisper, but is structure allows for migration and expadability as well. 
...
After containers are running, please refer to the following to kickstar the application:
...
- **Text-to-Speech:** Local synthesis using one of the Piper TTS avialable models
